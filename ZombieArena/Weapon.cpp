...
Weapon::Weapon()
{
	...
	m_rifle= new LSound("sound/rifle.wav"); // Prepare the shoot sound
...
bool Weapon::Shoot(float startX, float startY, float xTarget, float yTarget)
{
	...
	if ((dt > m_fireRate))
	{
		...
		case WeaponType::SHOTGUN:
			if (shootBullet(startX, startY, xTarget, yTarget, -1.5f)) count++;
			if (shootBullet(startX, startY, xTarget, yTarget,  1.5f)) count++;
		...
		case WeaponType::RIFLE:
			if (shootBullet(startX, startY, xTarget, yTarget, 0)) m_rifle->play();
			break;
		}
	...
	for (uint i = 0; i < BULLET_SPRITES; i++)
	{
		if (m_bullets[i].isInFlight())
		{
			if (m_bullets[i].getPosition().intersects(apZombie->getPosition()))
			{
				...
				if (apZombie->hit(m_damage)) // Register the hit and see if it was a kill
				{
